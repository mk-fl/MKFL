...
from gnn_helpers.utils import gen_graph_data, read_gs_4_gnn, read_mapping, read_mapping_inverse, read_gs, read_json_4_gnn, read_json_4_wl
    ...
    for family in families:
        ...
        if not os.path.isdir(path) :
            ...
            for file in filenames:
                # import pdb; pdb.set_trace()
                edges, nodes, vertices, edge_labels = [], [], [], []
                wl_graph = None
                if file.endswith(".gs"):
    ...
    for family in families:
        ...
        if not os.path.isdir(path) :
            ...
            if family == "cleanware":
                ...
                # fdf = pd.read_csv(f'./SemaClassifier/classifier/GNN/gnn_helpers/filtered_sorted_metadata.csv')
                fdf = pd.read_csv(f'./SemaClassifier/classifier/GNN/gnn_helpers/bodmas2_sorted_metadata.csv')
